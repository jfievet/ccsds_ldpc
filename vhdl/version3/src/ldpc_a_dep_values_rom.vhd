...
architecture rtl of ldpc_a_dep_values_rom is
    type rom_t is array (0 to A_DEP_VALUES_COUNT - 1) of std_logic_vector(LDPC_MESSAGE_INDEX_WIDTH - 1 downto 0);

    function unpack_values return rom_t is
        variable data : rom_t;
    begin
        for index in 0 to A_DEP_VALUES_COUNT - 1 loop
            data(index) := A_DEP_VALUES_BITS((index + 1) * LDPC_MESSAGE_INDEX_WIDTH - 1 downto index * LDPC_MESSAGE_INDEX_WIDTH);
        end loop;
        return data;
    ...
    begin
        if rising_edge(clock_i) then
            if rd_en_i = '1' then
                ...
                if address_index < A_DEP_VALUES_COUNT then
