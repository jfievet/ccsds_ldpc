...
begin
  active_o <= '1' when state /= idle_s else '0';
          ...
          case state is
            ...
            when prefetch_s =>
              if LDPC_N > 1 then
                request_index <= 1;
                if 1 < LDPC_K then
                  message_rd_addr_o <= std_logic_vector(to_unsigned(1, LDPC_MESSAGE_INDEX_WIDTH));
                else
            ...
            when stream_s =>
              ...
              if output_index = LDPC_N - 1 then
                ...
                if request_index < LDPC_N - 1 then
                  ...
                  if request_index + 1 < LDPC_K then
                    message_rd_addr_o <= std_logic_vector(to_unsigned(request_index + 1, LDPC_MESSAGE_INDEX_WIDTH));
                  else
