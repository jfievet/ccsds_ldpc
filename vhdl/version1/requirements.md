...
This function shall generate VHDL constants or memory initialization data that are:
...
Provide a single `compile.do` script to compile all VHDL files and run the simulation in Questa.
...
If Octave or MATLAB is not available, an equivalent offline generator may also be provided, but the generated vectors must match the same reference behavior.
...
A large constants file by itself is acceptable only if it synthesizes into practical FPGA memory resources.
...
- `sim`: `.do`, generated vectors, and helper generation scripts
