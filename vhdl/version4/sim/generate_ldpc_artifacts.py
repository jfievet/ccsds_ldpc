...
class Constants:
    ...
    forward_active_pivots: list[int]
    forward_target_masks: list[int]
    backward_target_rows: list[list[int]]
    backward_target_masks: list[int]
...
def build_constants_from_h(configuration: LdpcConfiguration) -> Constants:
    ...
    forward_active_pivots = [
        pivot for pivot in range(m)
        if forward_swap_rows[pivot] != pivot or len(forward_target_rows[pivot]) > 0
    ]
    forward_target_masks = masks_from_rows(forward_target_rows)
    backward_target_masks = masks_from_rows(backward_target_rows)
    ...
    return Constants(
        ...
        forward_active_pivots=forward_active_pivots,
        forward_target_masks=forward_target_masks,
        backward_target_rows=backward_target_rows,
        backward_target_masks=backward_target_masks,
    )
...
def format_vector_literal(value: int, width: int, indent: str = "  ") -> str:
    return format_hex_literal(format(value, f"0{width}b"), indent)


def format_rom_entries(values: list[int], width: int, indent: str = "        ") -> str:
    lines: list[str] = []
    for index, value in enumerate(values):
        suffix = "," if index + 1 < len(values) else ""
        lines.append(f"{indent}{index} => {format_vector_literal(value, width, indent)}{suffix}")
    return "\n".join(lines)
...
def write_values_rom(
        ...
        data_width: int,
        rom_style: str,
        read_lane_count: int = 1,
) -> None:
        rom_length = len(values)
        rom_entries = format_rom_entries(values, data_width)

        if read_lane_count == 1:
            data_port_text = f"        rd_data_o : out std_logic_vector({data_width_name} - 1 downto 0)"
            data_signal_text = f"    signal rd_data : std_logic_vector({data_width_name} - 1 downto 0) := (others => '0');"
            data_assign_text = "    rd_data_o <= rd_data;"
            data_read_text = f"""                if address_index < ROM_LENGTH then
            ...
            data_assign_lines = []
            data_read_lines = []
            for lane_index in range(read_lane_count):
                separator = ";" if lane_index < read_lane_count - 1 else ""
                data_port_lines.append(
                    f"        rd_data_{lane_index}_o : out std_logic_vector({data_width_name} - 1 downto 0){separator}"
                )
                data_signal_lines.append(
                    f"    signal rd_data_{lane_index} : std_logic_vector({data_width_name} - 1 downto 0) := (others => '0');"
                )
                data_assign_lines.append(f"    rd_data_{lane_index}_o <= rd_data_{lane_index};")
                data_read_lines.append(
                    f"""                if address_index + {lane_index} < ROM_LENGTH then
                    rd_data_{lane_index} <= rom(address_index + {lane_index});
                else
                    rd_data_{lane_index} <= (others => '0');
                end if;"""
                )

            data_port_text = "\n".join(data_port_lines)
            data_signal_text = "\n".join(data_signal_lines)
            data_assign_text = "\n".join(data_assign_lines)
            data_read_text = "\n".join(data_read_lines)
...
architecture rtl of {entity_name} is
    constant ROM_LENGTH : natural := {rom_length};
    type rom_t is array (0 to ROM_LENGTH - 1) of std_logic_vector({data_width_name} - 1 downto 0);
    signal rom     : rom_t := (
{rom_entries}
    );
{data_signal_text}
...
def write_banked_mask_rom(
        ...
        rom_length = len(masks)
        bank_count = (max((mask.bit_length() for mask in masks), default=0) + bank_width - 1) // bank_width
        if bank_count == 0:
            bank_count = 1
        ...
        read_lines: list[str] = []
        bank_rom_lines: list[str] = []

        for bank_index in range(bank_count):
            separator = ";" if bank_index < bank_count - 1 else ""
            port_lines.append(
                f"        rd_data_{bank_index}_o : out std_logic_vector({bank_width} - 1 downto 0){separator}"
            )
            signal_lines.append(
                f"    signal rom_{bank_index} : rom_t := (\n{format_rom_entries([(mask >> (bank_index * bank_width)) & ((1 << bank_width) - 1) for mask in masks], bank_width)}\n    );"
            )
            signal_lines.append(
                f"    signal rd_data_{bank_index} : std_logic_vector({bank_width} - 1 downto 0) := (others => '0');"
            )
            assign_lines.append(f"    rd_data_{bank_index}_o <= rd_data_{bank_index};")
            read_lines.append(
                f"""                if address_index < ROM_LENGTH then
                    rd_data_{bank_index} <= rom_{bank_index}(address_index);
                else
                    rd_data_{bank_index} <= (others => '0');
                end if;"""
            )
            bank_rom_lines.append(f"    attribute rom_style of rom_{bank_index} : signal is \"{rom_style}\";")
...
architecture rtl of {entity_name} is
    constant ROM_LENGTH : natural := {rom_length};
    type rom_t is array (0 to ROM_LENGTH - 1) of std_logic_vector({bank_width} - 1 downto 0);
{chr(10).join(signal_lines)}

    attribute rom_style : string;
{chr(10).join(bank_rom_lines)}
begin
...
def write_split_vhdl_tables(constants: Constants, output_dir: Path, output_prefix: str) -> list[Path]:
    ...
    rom_paths = [
        ...
        output_dir / "ldpc_fwd_target_masks_rom.vhd",
        output_dir / "ldpc_bwd_target_masks_rom.vhd",
    ]
    ...
    solver_body = "".join(
        [
            ...
            format_packed_constant("FWD_ACTIVE_PIVOTS_BITS", "FWD_ACTIVE_PIVOTS_COUNT", "LDPC_ROW_INDEX_WIDTH", constants.forward_active_pivots, parity_row_width),
    ...
    write_banked_mask_rom(
        rom_paths[5],
        "ldpc_fwd_target_masks_rom",
        config_package_name,
        constants.forward_target_masks,
        "block",
    )
    write_banked_mask_rom(
        rom_paths[6],
        "ldpc_bwd_target_masks_rom",
        config_package_name,
        constants.backward_target_masks,
        "block",
    )

    for obsolete_path in [output_dir / "ldpc_fwd_target_values_rom.vhd", output_dir / "ldpc_bwd_target_values_rom.vhd"]:
