...
typedef struct {
    ...
    /* LDPC config (CCSDS AR4JA) */
    int ldpc_cfg;            /* selection id (1..9), see c/qc_encoder_all */
    int ldpc_max_iters;      /* max decoding iterations */
    float ldpc_oms_offset;   /* offset for Offset-Min-Sum decoder */
    const char *ldpc_h_path; /* optional override path to H_*.mat (NULL = auto from encoder config) */

    /* LLR quantization */
    int llr_width_bits;      /* 0 = float (no quantization), else 3/4/5/6 */
    float llr_clip;          /* absolute clip value before quantization (used when llr_width_bits != 0) */

    /* Optional dump of (quantized) channel LLRs fed to the LDPC decoder. */
    const char *llr_out_path;     /* NULL = disabled, "auto" handled by CLI */
    int llr_out_frames;           /* number of frames to dump (<=0 means dump all) */
} qpsk_ber_cfg_t;
...
/* End-to-end BER measurement for one Eb/N0 point, with LDPC (encoder + OMS decoder). */
